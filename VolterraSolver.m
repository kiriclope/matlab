function [ y ] = VolterraSolver( h,f,ker )
% Matlab Code to solve Volterra Integral Equations with Convolution Kernel using Laplace Transforms
...
% VolterraSolver accepts h(x), f(x) and ker(x-t) from the user and solves
% the integral equation and gives the output in y(x).
% Example :
% Take the following Volterra 1st kind nonhomogeneous Integral
% Equation with difference kernel
% sin(x)=int(exp(x-t)y(t),0,x)dt
% Command window :
% >> syms x t s
% >> VolterraSolver(0,sin(x),exp(x-t))
% ans = cos(x) - sin(x)
